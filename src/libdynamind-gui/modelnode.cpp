...
void ModelNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
    ...
    if(this->visible){
        ...
        else if(this->getName()=="Import WSUD Layout (.msf)")
        {
            ...
            l = img.width();
            painter->drawImage(0,0,img);
            QRectF *rect = new QRectF(0,h-40,400,70);
            painter->drawText(*rect,QString("Import MUSIC File (.msf)\n") + this->getParameterAsString("Filename").c_str());
        ...
        else if(this->getName()=="Import Land Cover Map")
        {
            ...
            l = img.width();
            painter->drawImage(0,0,img);
            painter->drawText(10,h-10,QString("Import Land Cover Map"));
